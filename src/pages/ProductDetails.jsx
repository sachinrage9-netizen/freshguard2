...
import {getProductRisk,getSensorReadings,getStorageById} from '../api/client';
...
export default function ProductDetails({products}){
  ...
  useEffect(()=>{
    if(!p||p.locationId==null){setHistoryReadings(null);setStorage(null);return}
    let cancelled=false;
    const since=new Date(Date.now()-24*60*60*1000).toISOString().replace('T',' ').slice(0,19);
    Promise.all([
      getSensorReadings({storage_location_id:p.locationId,since}),
      getStorageById(p.locationId),
    ]).then(([readings,detail])=>{if(cancelled)return;setHistoryReadings(Array.isArray(readings)&&readings.length?readings.slice(0,24).reverse():null);setStorage(detail)}).catch(()=>{if(!cancelled){setHistoryReadings(null);setStorage(null)}});
    return()=>{cancelled=true};
