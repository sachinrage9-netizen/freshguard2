...
function riskPriority(risk, expiry){const level=risk?.risk_level;const days=risk?.days_remaining;if(level==='CRITICAL'||level==='HIGH')return 'High priority';if(level==='MODERATE')return 'Sell soon';if(days!=null&&days<=3)return 'Sell soon';if(expiry){const remaining=Math.ceil((new Date(expiry+'T00:00:00')-new Date())/(1000*60*60*24));if(remaining<=3)return 'Sell soon';}return 'Normal';}
function mapBackendProduct(p,locationTemps={},risks={}){const locationId=p.storage_location_id;const categoryId='cat-'+Math.max(0,['Dairy','Drinks','Frozen','Snacks','Bakery','Other'].indexOf(p.category));const risk=risks[p.product_code];return {id:p.product_code,productCode:p.product_code,name:p.name,category:p.category,categoryId,categoryPath:[p.category],quantity:p.quantity,expiry:p.expiry_date,location:p.storage_location,locationId,storageLocationId:locationId,priority:riskPriority(risk,p.expiry_date),temp:locationTemps[locationId]??null,history:[],maxTemperature:p.max_temperature,position_x:p.position_x,position_y:p.position_y,position_z:p.position_z,shelf:p.shelf,slot:p.slot,row:p.row,column:p.column,overallRisk:risk?.overall_risk,riskLevel:risk?.risk_level};}
function mapBackendLocation(x){return {id:x.id,backendId:x.id,name:x.name,type:x.type,sensor:!!x.sensor,sensorEnabled:!!x.sensor_enabled,temperature:x.temperature,humidity:x.humidity,lastUpdated:x.last_updated,productCount:x.product_count,width:x.width,height:x.height,depth:x.depth};}
...
export default function App(){
  ...
  const [productEdits,setProductEdits]=useState(()=>get('freshguard-product-edits',{}));
  ...
  useEffect(()=>localStorage.setItem('freshguard-product-edits',JSON.stringify(productEdits)),[productEdits]);
  ...
  useEffect(()=>{
    ...
    async function poll(){
      ...
      const riskMap={};if(Array.isArray(risks))risks.forEach(r=>{if(r?.product_code)riskMap[r.product_code]=r});
      const locationTemps={};
      if(Array.isArray(backendLocations)){backendLocations.forEach(x=>{locationTemps[x.id]=x.temperature});setLocations(backendLocations.map(mapBackendLocation));}
      if(Array.isArray(backendProducts)){
        setProducts(backendProducts.map(p=>({...mapBackendProduct(p,locationTemps,riskMap),...(productEdits[p.product_code]||{})})));
    ...
    poll();const id=setInterval(poll,5000);return()=>{cancelled=true;clearInterval(id)};
  },[productEdits]);

  const saveProductEdit=(productCode,changes)=>{
    setProductEdits(prev=>({...prev,[productCode]:{...(prev[productCode]||{}),...changes}}));
    setProducts(prev=>prev.map(p=>p.productCode===productCode||p.id===productCode?{...p,...changes}:p));
  };
  const login=u=>{localStorage.setItem('freshguard-session',JSON.stringify(u));setUser(u)};
  const props={products,setProducts,saveProductEdit,sensor,sensorStatus:backendConnected?'ready':'error',locations,categories,dashboard,user,backendConnected,alerts};
  ...
  return <Routes>
    ...
    <Route path="/*" element={<Protected user={user}><Layout user={user} backendConnected={backendConnected} dashboard={dashboard}><Routes>
      <Route path="/" element={<Dashboard {...props}/>}/><Route path="/inventory" element={<Inventory {...props}/>}/><Route path="/storage" element={<Storage locations={locations} setLocations={setLocations} products={products}/>}/><Route path="/product/:id" element={<ProductDetails {...props}/>}/><Route path="/product/:id/edit" element={<EditProduct {...props}/>}/><Route path="/add" element={<AddProduct {...props}/>}/><Route path="/alerts" element={<Alerts alerts={alerts}/>}/><Route path="/reports" element={<Reports sensor={sensor} locations={locations}/>}/><Route path="/settings" element={<Settings {...props} user={user} setUser={setUser} setLocations={setLocations} setCategories={setCategories} dashboard={dashboard} setDashboard={setDashboard}/>} />
